...
import com.example.lenta.databinding.ItemFileGridBinding
import java.io.File
    ...
    inner class FileViewHolder(private val binding: ItemFileGridBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: MediaItem) {
            ...
            if (item.isDirectory) {
                ...
                val firstMedia = if (!item.isNextcloud) {
                ...
                if (item.isNextcloud) {
                    ...
                    val loadTarget: Any = if (cachedFile.exists() && cachedFile.length() > 0) {
                        ...
                        when (item.type) {
                            MediaType.VIDEO -> {
                                val ncCacheKey = ThumbnailManager.getVideoThumbnailCacheKey(item)
                                val cached = File(ThumbnailManager.getVideoThumbnailDir(context.cacheDir), ncCacheKey)
                                if (cached.exists() && cached.length() > 0L) {
                                    cached
                                } else {
                                    val client = NextcloudClient(NextcloudPreferences(context))
                                    client.getPreviewUrl(item) ?: item.uriString
                                }
                            }
                            MediaType.CBZ -> {
                                val ncCacheKey = ThumbnailManager.getCbzCoverCacheKey(item)
                                val cached = ThumbnailManager.getCbzThumbnailDir(context.cacheDir)
                                    .listFiles { _, name -> name.startsWith(ncCacheKey) }
                                    ?.firstOrNull { it.length() > 0L }
                                if (cached != null) {
                                    cached
                                } else {
