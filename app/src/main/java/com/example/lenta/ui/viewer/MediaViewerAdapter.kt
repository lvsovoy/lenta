...
import com.example.lenta.databinding.ItemMediaCbzBinding
import com.example.lenta.databinding.ItemMediaGifBinding
import com.example.lenta.databinding.ItemMediaImageBinding
import com.example.lenta.databinding.ItemMediaVideoBinding
...
class MediaViewerAdapter(
    ...
    inner class ImageViewHolder(private val binding: ItemMediaImageBinding) : BaseMediaViewHolder(binding.root) {
        ...
        override fun bind(item: MediaItem, position: Int) {
            ...
            job?.cancel()
            job = scope.launch {
                val dataToLoad: Any = if (item.isNextcloud) {
                    val cachedResult = nextcloudClient.downloadToCache(binding.root.context, item)
    ...
    inner class VideoViewHolder(private val binding: ItemMediaVideoBinding) : BaseMediaViewHolder(binding.root) {
        ...
        override fun bind(item: MediaItem, position: Int) {
            ...
            binding.videoTouchOverlay.setOnTouchListener(tapListener)
            ...
            job = scope.launch {
                ...
                val mediaUri: Uri = if (item.isNextcloud) {
                    val cached = nextcloudClient.downloadToCache(binding.root.context, item)
                    if (cached.isSuccess) {
    ...
    inner class CbzViewHolder(val binding: ItemMediaCbzBinding) : BaseMediaViewHolder(binding.root) {
        ...
        override fun bind(item: MediaItem, position: Int) {
            ...
            job?.cancel()
            job = scope.launch {
                val comicFile: File = if (item.isNextcloud) {
                    val cached = nextcloudClient.downloadToCache(binding.root.context, item)
                    if (cached.isSuccess) cached.getOrThrow() else File(item.path)
                } else {
                    File(item.path)
                }

                val pages = CbzReader.getComicPages(binding.root.context, comicFile)
                pageFiles = pages
